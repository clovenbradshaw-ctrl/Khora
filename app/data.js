...
const DataTable = ({
  ...
  if (groupBy !== 'none') {
    grouped = {};
    sorted.forEach(row => {
      const gk = getVal(row, groupBy);
      // Support array group keys — row appears under each group
      if (Array.isArray(gk)) {
        const keys = gk.length > 0 ? gk : ['Untagged'];
        keys.forEach(k => {
          const key = k || 'Other';
    ...
    // Sort group keys alphabetically (except 'Untagged' goes last)
    const sortedGrouped = {};
...
const NotesTable = ({
  ...
  const isMobile = useIsMobile();
  const [mobileGroupBy, setMobileGroupBy] = useState('none');
  const NOTE_COLS = [{
    key: 'title',
    label: 'Title',
    fixed: true
  ...
  const getNoteVal = (row, key) => {
    ...
    if (key === 'tags_list') return (row.tags || []).map(t => t.displayName);
    return row[key] || '';
  };
  ...
  return /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    ...
    /*#__PURE__*/React.createElement(React.Fragment, null,
      ...
      /*#__PURE__*/React.createElement("div", { style: { display: 'flex', gap: 6, marginBottom: 8, flexWrap: 'wrap' } },
        NOTE_GROUPS.map(g => /*#__PURE__*/React.createElement("button", {
          key: g.k,
          className: mobileGroupBy === g.k ? 'b-pri b-xs' : 'b-gho b-xs',
          onClick: () => setMobileGroupBy(g.k)
        }, g.l))
      ),
      (() => {
        ...
        if (mobileGroupBy === 'none') {
          return /*#__PURE__*/React.createElement("div", { style: { display: 'flex', flexDirection: 'column', gap: 8 } },
            noteRows.map(renderCard));
        }
        // Build grouped sections
        const mGroups = {};
        noteRows.forEach(row => {
          const val = getNoteVal(row, mobileGroupBy);
          if (Array.isArray(val)) {
            const keys = val.length > 0 ? val : ['Untagged'];
            keys.forEach(k => { if (!mGroups[k]) mGroups[k] = []; mGroups[k].push(row); });
          } else {
            const k = val || 'Other';
        ...
        const sortedKeys = Object.keys(mGroups).sort((a, b) => {
          if (a === 'Untagged') return 1;
          if (b === 'Untagged') return -1;
          return a.localeCompare(b);
        });
        return /*#__PURE__*/React.createElement("div", { style: { display: 'flex', flexDirection: 'column', gap: 12 } },
          sortedKeys.map(gk => /*#__PURE__*/React.createElement("div", { key: gk },
            /*#__PURE__*/React.createElement("div", {
              style: { fontSize: 12, fontWeight: 700, color: 'var(--tx-1)', padding: '6px 0 4px', borderBottom: '1px solid var(--border-0)', marginBottom: 6 }
            }, gk, ' (', mGroups[gk].length, ')'),
            /*#__PURE__*/React.createElement("div", { style: { display: 'flex', flexDirection: 'column', gap: 8 } },
              mGroups[gk].map(renderCard))
          ))
        );
      })()
    ) :
